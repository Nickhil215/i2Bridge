...
class BaseAnalyzer(ABC):
    ...
    def _extract_function_info(self, node: nodes.FunctionDef, source: str) -> FunctionInfo:
        ...
        return FunctionInfo(
            ...
            args=args,
            returns=returns,
            exec_cmd=class_name + "." + f"{node.name}({', '.join(arg.name for arg in node.args.args)})" if class_name else None,
            docstring=docstring,
            comments=comments,
    ...
    def generate_report(self) -> str:
      ...
      if self.code_component_info:
        for file_path, core_components in sorted(self.code_component_info.items()):
          if not core_components.name.startswith("_"):
            report.append(f"\n  {file_path}:")
            report.extend([
